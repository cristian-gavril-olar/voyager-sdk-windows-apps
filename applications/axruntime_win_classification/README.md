...
### Method 2: Windows Explorer (Easy Launch)

**After completing the full Windows Getting Started Guide**, you can also run the application by:

1. Navigate to the `applications/axruntime_win_classification/` folder in Windows Explorer
2. Double-click on `start_classification.bat`

This batch file will automatically:
- Activate the Python virtual environment
- Use the default camera (camera ID 0)
- Load the ResNet50 ImageNet model
- Start the classification application

**Note**: This method requires that you have completed all prerequisites from the Windows Getting Started Guide, including downloading the ResNet50 model using `./download_prebuilt.py resnet50-imagenet-onnx`.
...
| `--labels` | str | Yes | - | Path to labels file |
...
The `--labels` argument is required and must point to a valid labels file. The labels file should contain one label per line, with line numbers corresponding to class indices.

For ImageNet models, you can use the default ImageNet labels file located at:
```
...
- **Labels file not found**: Ensure the `--labels` argument points to a valid file
