...
# these values are an artefact of the model - may need adjustment for specific detection models
mean = [0.485, 0.456, 0.406]
stddev = [0.229, 0.224, 0.225]

# Generate consistent colors for each class
def _generate_colors(num_classes=100):
    """Generate consistent colors for each class."""
...
_DEFAULT_LABELS = os.path.expandvars(
    "$AXELERA_FRAMEWORK/ax_datasets/labels/coco_labels.txt"
)
...
parser.add_argument(
    ...
    type=Path,
    default="images_out",
    help="Output directory for images with drawn bounding boxes (default: %(default)s)",
)
parser.add_argument(
    "--save-images",
    action="store_true",
    help="Save images with drawn bounding boxes to output directory",
)
parser.add_argument(
    "-v",
    "--verbose",
...
def _non_max_suppression(boxes, scores, class_ids, conf_threshold=0.25, iou_threshold=0.45, max_detections=100):
    """Apply Non-Maximum Suppression to detection results with improved filtering."""
    if len(boxes) == 0:
...
def _draw_detections(image_path: Path, boxes, scores, class_ids, labels, output_dir: Path = None):
    ...
    image = cv2.imread(str(image_path))
    if image is None:
        LOG.warning(f"Could not load image for drawing: {image_path}")
        return None
    ...
    for box, score, class_id in zip(boxes, scores, class_ids):
        ...
        class_name = labels[int(class_id)] if int(class_id) < len(labels) else f"class_{int(class_id)}"
    ...
    output_filename = f"detected_{image_path.name}"
    output_path = output_dir / output_filename
    
    success = cv2.imwrite(str(output_path), image)
    if success:
        LOG.info(f"Saved image with detections: {output_path}")
        return output_path
    else:
        LOG.error(f"Failed to save image: {output_path}")
        return None
...
def _postproc_detection(image_path: Path, outputs: list, labels: list[str], 
                       ...
                       max_detections: int = 100, output_dir: Path = None, save_images: bool = False):
    """
    Postprocess detection outputs to extract bounding boxes, scores, and class IDs.
    
    This function assumes YOLO-style output format:
    ...
    for output, info in zip(outputs, output_infos):
        ...
        if len(out.shape) == 2:
            ...
            # Shape: [1, num_detections, 85] or [batch, height, width] for feature maps
            if out.shape[1] > 100:  # Likely a feature map, need to decode differently
                LOG.warning(f"Feature map output detected: {out.shape}. This may require anchor-based decoding.")
                continue
            detections = out[0]
        elif len(out.shape) == 4:
            # Shape: [batch, anchors, height, width] - feature map format
            LOG.warning(f"4D output detected: {out.shape}. This requires anchor-based decoding which is not implemented.")
            continue
        else:
        ...
        # Debug: print raw values to understand the format
        if LOG.isEnabledFor(logging.DEBUG):
            LOG.debug(f"Raw detection sample: {detections[0] if len(detections) > 0 else 'None'}")
            LOG.debug(f"Detection shape: {detections.shape}")
        ...
        if detections.shape[1] > 5:
            ...
            print(f"DEBUG: Raw objectness range: [{objectness.min():.3f}, {objectness.max():.3f}]")
            print(f"DEBUG: Raw class scores range: [{class_scores.min():.3f}, {class_scores.max():.3f}]")
            
            # Apply sigmoid to class scores if they seem to be raw logits
            if np.any(class_scores > 1) or np.any(class_scores < 0):
                print("DEBUG: Applying sigmoid to class scores")
                class_scores = 1 / (1 + np.exp(-np.clip(class_scores, -500, 500)))  # sigmoid with clipping
            
            # Apply sigmoid to objectness if it seems to be raw logits
            if np.any(objectness > 1) or np.any(objectness < 0):
                print("DEBUG: Applying sigmoid to objectness scores")
                objectness = 1 / (1 + np.exp(-np.clip(objectness, -500, 500)))  # sigmoid with clipping
            
            # For models with separate objectness and class scores
            max_class_scores = np.max(class_scores, axis=1)
            scores = objectness * max_class_scores
            class_ids = np.argmax(class_scores, axis=1)
            
            print(f"DEBUG: Final score range: [{scores.min():.3f}, {scores.max():.3f}]")
        else:
            # For models where objectness is the final score
            print(f"DEBUG: Raw objectness range: [{objectness.min():.3f}, {objectness.max():.3f}]")
            
            # Apply sigmoid if scores seem to be raw logits
            if np.any(objectness > 1) or np.any(objectness < 0):
                print("DEBUG: Applying sigmoid to objectness scores")
                objectness = 1 / (1 + np.exp(-np.clip(objectness, -500, 500)))  # sigmoid with clipping
            scores = objectness
        ...
        # Debug coordinate ranges to understand the format
        print(f"DEBUG: Box coordinate ranges: x=[{boxes[:, 0].min():.3f}, {boxes[:, 0].max():.3f}], "
              f"y=[{boxes[:, 1].min():.3f}, {boxes[:, 1].max():.3f}], "
              f"w=[{boxes[:, 2].min():.3f}, {boxes[:, 2].max():.3f}], "
              f"h=[{boxes[:, 3].min():.3f}, {boxes[:, 3].max():.3f}]")
        print(f"DEBUG: Image dims: orig={orig_w}x{orig_h}, model={model_w}x{model_h}")
        print(f"DEBUG: Sample boxes: {boxes[:3]}")
        
        x_center, y_center, width, height = boxes[:, 0], boxes[:, 1], boxes[:, 2], boxes[:, 3]
        
        # Based on the debug output, let's handle coordinate conversion more systematically
        # The model appears to output coordinates in model input scale (e.g., 0-640 for a 640x640 model)
        
        # First, let's determine the coordinate scale
        max_coord = np.max(np.abs(boxes[:, :2]))  # Check x,y coordinates
        max_size = np.max(boxes[:, 2:])  # Check width, height
        
        print(f"DEBUG: Max coordinate: {max_coord:.3f}, Max size: {max_size:.3f}")
        
        if max_coord <= 1.0 and max_size <= 1.0:
            # Normalized coordinates [0, 1]
            print("DEBUG: Using normalized coordinate interpretation")
            x1 = (x_center - width / 2) * orig_w
            y1 = (y_center - height / 2) * orig_h
            x2 = (x_center + width / 2) * orig_w
            y2 = (y_center + height / 2) * orig_h
        else:
            # Coordinates are likely in model input scale
            print("DEBUG: Using model input scale interpretation")
            
            # Convert from center-based to corner-based in model space
        ...
        print(f"DEBUG: Sample converted coords: ({x1[0]:.1f}, {y1[0]:.1f}) to ({x2[0]:.1f}, {y2[0]:.1f})")
        ...
        # Create boxes in [x1, y1, x2, y2] format
    ...
    # Apply NMS
    if len(all_boxes) > 0:
        # Debug: print some coordinate information
        LOG.debug(f"Sample coordinates before NMS: {all_boxes[:3] if len(all_boxes) > 0 else 'None'}")
        LOG.debug(f"Image dimensions: orig={orig_w}x{orig_h}, model={model_w}x{model_h}")
        
        # Convert to format expected by cv2.dnn.NMSBoxes [x, y, w, h]
        # Ensure all coordinates are positive and within bounds
        x1, y1, x2, y2 = all_boxes[:, 0], all_boxes[:, 1], all_boxes[:, 2], all_boxes[:, 3]
        
        # Calculate width and height, ensuring they're positive
        widths = np.maximum(x2 - x1, 1)
        heights = np.maximum(y2 - y1, 1)
        
        boxes_xywh = np.column_stack([x1, y1, widths, heights])
        
        # Filter out boxes that are completely outside the image
        valid_boxes_mask = (x1 < orig_w) & (y1 < orig_h) & (x2 > 0) & (y2 > 0)
        
        if np.any(valid_boxes_mask):
            boxes_xywh = boxes_xywh[valid_boxes_mask]
            filtered_scores = all_scores[valid_boxes_mask]
            filtered_class_ids = all_class_ids[valid_boxes_mask]
            
            final_boxes, final_scores, final_class_ids = _non_max_suppression(
                boxes_xywh, filtered_scores, filtered_class_ids, conf_threshold, nms_threshold, max_detections
            )
        else:
            final_boxes, final_scores, final_class_ids = np.array([]), np.array([]), np.array([])
        ...
        if len(final_boxes) > 0:
            ...
            # Draw and save image with detections if requested
            if save_images and output_dir:
                _draw_detections(image_path, final_boxes, final_scores, final_class_ids, labels, output_dir)
        else:
            print(f"{image_path}: No detections after NMS")
            
            # Still save the original image if requested (without detections)
            if save_images and output_dir:
                output_dir.mkdir(parents=True, exist_ok=True)
                output_filename = f"no_detections_{image_path.name}"
                output_path = output_dir / output_filename
                shutil.copy2(image_path, output_path)
                LOG.info(f"Saved image without detections: {output_path}")
    else:
    ...
    with Context() as ctx:
        ...
        try:
            ...
            for in_frameno, image_path in enumerate(input_paths):
                ...
                image_dims_list.append(image_dims)
                
                if in_frameno >= prefill:
                    next_ready = out_frameno % len(workers)
                    out_path, outs, dims = workers[next_ready].pop()
                    _postproc_detection(out_path, outs, labels, output_infos, dims, conf_threshold, nms_threshold, max_detections, output_dir, save_images)
                    out_frameno += 1
...
def main(args: argparse.Namespace):
    ...
    # Try to load labels, with fallback options
    labels = []
    if args.labels and args.labels.exists():
        labels = args.labels.read_text().splitlines()
    else:
        # Try COCO labels as fallback
        coco_labels_path = Path(os.path.expandvars("$AXELERA_FRAMEWORK/ax_datasets/labels/coco_labels.txt"))
